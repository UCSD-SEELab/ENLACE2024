...
'''
def unpack_user_uploaded_data(uuid,upload_dir,subject,unpack_dir):
	# Source directory for the user's data:
	src_user_dir = os.path.join(upload_dir,subject,uuid);
	...
	dst_user_dir = os.path.join(unpack_dir,subject,uuid);
	...
	for (si,subdir) in enumerate(subdirs):
		print("instance %d (out of %d). Timestamp: %s" % (si+1,len(subdirs),subdir));
...
'''
def collect_user_data_and_save_csv_file(uuid,unpack_dir,subject,uuids_csvs_dir,subject_csv_dir,interest_labels,sensors=None,add_time_of_day_features=True):
	if sensors is None:
		sensors				= get_default_list_of_sensors();
		pass;
		
	uuid_unpack_dir			= os.path.join(unpack_dir,subject,uuid);
	...
	write_user_features_and_labels_to_csv(uuids_csvs_dir,subject_csv_dir,uuid,timestamps,X,Y,missing_label_mat,label_sources,feat_names,label_names);
...
def convert_feedback_file_to_labels_file(src_instance_dir,dst_instance_dir):
	...
	with open(label_file,'wt') as fid:
...
def write_user_features_and_labels_to_csv(uuids_csvs_dir,subject_csv_dir,uuid,timestamps,X,Y,missing_label_mat,label_sources,feat_names,label_names):
	print("="*40);
	...
	out_file				= os.path.join(uuids_csvs_dir,subject_csv_dir,'%s.features_labels.csv.gz' % uuid);
